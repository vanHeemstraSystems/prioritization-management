<template>
    <div class="controls">
        <ThePlayButton class="playback"
            @click="$emit(flowchartStore.exploringDuringPlayback ? 'stopExplorationDuringPlayback' : 'togglePlayback')" />
        <div ref="chapters" class="chapters"
            :class="{ shadow: !viewStore.introPanelVisible, playing: flowchartStore.playbackActive }">
            <div class="drawer">
                <span>
        ...
        <PrimaryButton class="jump"
            :class="{ visible: flowchartStore.movedAwayFromNarration, shadow: !viewStore.introPanelVisible }"
            :state="flowchartStore.jumpActionAvailable ? 'highlighted' : 'disabled'" icon="jump"
